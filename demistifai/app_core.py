...
ss = None  # will be bound to st.session_state at init time

nav_labels_by_key: Dict[str, str] = {}
label_to_stage_key: Dict[str, str] = {}
nav_pages: List[str] = []


def initialize_app() -> tuple[Dict[str, Any], Any]:
    """Ensure shared Streamlit state, CSS, and defaults are ready."""

    global _initialized, state, s, ss, nav_labels_by_key, label_to_stage_key, nav_pages

    if _initialized and state is not None and ss is not None:
        return state, ss
    ...
    nav_labels_by_key = {key: label for key, label in stage_labels}
    label_to_stage_key = {label: key for key, label in stage_labels}
    nav_pages = [label for _, label in stage_labels]

    current_stage_key = run_state.get("active_stage", STAGES[0].key)
    ss.setdefault("demai_top_nav", nav_labels_by_key.get(current_stage_key, nav_pages[0]))
    ...
    ss.setdefault("labeled", starter_dataset_copy())
    ss.setdefault("incoming", [])
...
def get_stage_key_for_label(label: str) -> str:
    initialize_app()
    return label_to_stage_key.get(label, STAGES[0].key)


def get_nav_label_for_stage(stage_key: str) -> str:
    initialize_app()
    return nav_labels_by_key.get(stage_key, stage_key)


def get_active_stage_key() -> str:
    initialize_app()
    assert s is not None
    run_state = s.setdefault("run", {})
    return run_state.get("active_stage", STAGES[0].key)
...
def render_stage(stage_key: Optional[str] = None) -> None:
    """Render the requested stage, defaulting to the active state."""
    ...
    run_state = s.setdefault("run", {})
    active_stage = stage_key or run_state.get("active_stage") or STAGES[0].key
...
def render_stage_by_label(label: str) -> None:
    """Render a stage by its navigation label."""

    stage_key = get_stage_key_for_label(label)
    render_stage(stage_key)
