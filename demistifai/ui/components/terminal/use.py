...
_TOKEN_REGEX = re.compile(
    r"(Prediction|Confidence|Recommendation|Spam|Safe|Inbox|threshold|email|title|body)",
    re.IGNORECASE,
)
...
(function(){
  ...
  const splitSegments = (line) => {
    ...
    // token emphasize for "use" stage
    const parts = [];
    const re = /(Prediction|Confidence|Recommendation|Spam|Safe|Inbox|threshold|email|title|body)/gi;
    let idx = 0, m;
