...
# --- Content for the TRAIN stage (short + progressive) ---
...
<div class="wrap-__SFX__">
  <div id="__DOMID__" class="term-__SFX__" role="region" aria-label="Train terminal animation">
    __SKIP__
...
(function(){
  ...
  const splitSegments = (line) => {
    ...
    // token emphasize for training stage
    const parts = [];
    const re = /(AI system|objective|labels?|patterns?|generalize|spam|safe|accuracy|precision|recall|epochs?)/gi;
    let idx = 0, m;
