...
def _render_training_examples_preview() -> None:
    """Show a small sampler of labeled emails with guardrail cues."""

    st.markdown(
        textwrap.dedent(
            """
            <div style="display:flex;flex-direction:column;gap:0.35rem;margin:0.85rem 0 0.65rem;">
    ...
    if cards_html_parts:
        ...
        st.markdown(
            textwrap.dedent(
                f"""
                <div style="width:100%;display:grid;grid-template-columns:repeat(auto-fit,minmax(190px,1fr));gap:0.75rem;">
                    {cards_html}
                </div>
                """
            ).strip(),
    ...
    st.markdown(
        f"<div style='margin-top:0.65rem;'>{chip_html}</div>",
        unsafe_allow_html=True,
    )
    st.caption(
        "These are the kinds of cues the model is likely to notice. You tuned their presence in Prepare."
    )
