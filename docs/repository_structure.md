...
- `__init__.py` – Aggregates stage-focused packages (intro, overview, data, train, evaluate, use) plus shared utilities so pages can import components from a single namespace.【F:demistifai/ui/components/__init__.py†L1-L70】
- `intro/` – Welcome-stage hero layout and lifecycle markup that frame the Article 3 walkthrough (`lifecycle.py`). Add new intro UI in this package to keep the hero experience cohesive.【F:demistifai/ui/components/intro/__init__.py†L1-L13】【F:demistifai/ui/components/intro/lifecycle.py†L1-L112】
- `overview/` – Mission briefings and architecture diagram helpers used across the Welcome and Overview stages (`overview_mission.py`, `arch_demai.py`). Extend this folder when adding more context cards for the system snapshot.【F:demistifai/ui/components/overview/__init__.py†L1-L22】【F:demistifai/ui/components/overview/arch_demai.py†L1-L146】
- `data/` – Prepare-stage component bundle covering dataset comparison markup, CLI callouts, review dashboards, and PII cleanup surfaces (`builder.py`, `data_review.py`, `pii.py`, `pii_indicators.py`). Use these helpers to keep dataset workflows consistent across pages.【F:demistifai/ui/components/data/__init__.py†L1-L28】【F:demistifai/ui/components/data/builder.py†L1-L156】【F:demistifai/ui/components/data/data_review.py†L1-L158】【F:demistifai/ui/components/data/pii.py†L1-L116】【F:demistifai/ui/components/data/pii_indicators.py†L1-L25】
- `train/` – Training launchpad cards, guardrail previews, animation scaffolding, numeric clue explainers, and language mix chips (`train_intro.py`, `train_animation.py`, `guardrails.py`, `numeric_clues.py`, `language_mix.py`). Put Train stage expansions in this package to reuse shared CSS and chips.【F:demistifai/ui/components/train/__init__.py†L1-L32】【F:demistifai/ui/components/train/train_animation.py†L1-L122】【F:demistifai/ui/components/train/guardrails.py†L1-L93】【F:demistifai/ui/components/train/numeric_clues.py†L1-L123】
...
- `use/` – Use-stage inbox preview and results cards that orchestrate batch predictions, audit logging, and Nerd Mode diagnostics (`incoming_preview.py`, `batch_results.py`). Centralise future live-routing UI here.【F:demistifai/ui/components/use/__init__.py†L1-L6】【F:demistifai/ui/components/use/incoming_preview.py†L1-L74】【F:demistifai/ui/components/use/batch_results.py†L1-L120】
...
- `evaluate.py` – Evaluation dashboards for metrics, threshold management, and governance summaries.【F:pages/evaluate.py†L1-L375】
- `use.py` – Live classification console with autonomy toggles, adaptiveness, and routing diagnostics.【F:pages/use.py†L1-L247】
